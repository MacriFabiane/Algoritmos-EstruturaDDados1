...
void inserir_primeiro(int key, ListaE *l){
    Cell *aux= criar_celula(key); // Nova célula
...
int remover(int key, ListaE *l){
    Cell *auxA, *auxR = NULL; // células auxiliares

    if (!listaE_vazia(l)){ // apontar o auxA para a cabeça da lista
        ...
        if(l->head->item == key){
            ...
            while((auxR != NULL) && (auxR->item != key)){
                //if (auxP != NULL)
                    auxA = auxR; // Guardar o endereço auxA
