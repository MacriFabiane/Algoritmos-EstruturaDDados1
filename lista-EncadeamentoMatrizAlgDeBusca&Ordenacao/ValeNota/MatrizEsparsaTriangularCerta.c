...
int triangularInf (Spa_Mat *mat, int c, int l){
    int i = 0;
    int continf=0;
    Cell* aux;
...
void triangular (Spa_Mat *mat, int c, int l){
    int i =0;
    int continf=0, contsup=0;
    Cell* aux;
    ...
    for (i=0;i<l;i++){
        aux=mat->lin[i]->head;
            ...
            {
                if(i==aux->col){
                    if(mat->lin[i]==NULL || mat->lin[i]->head->col==i){
                            contsup++;
                        }
    ...
    continf=triangularInf(mat, c, l);

    if((continf!=(l-1) && contsup!=l)){
