...
static void inserir_primeiro(ListaE *l, int col, int item){
 ...
 Spa_Mat * converter (int l,int c, int** mat_bi){
     int i, j;
...
int cont_N_null (Spa_Mat *mat, int l, int c){
    int i, j, cont=0;
    Cell* aux;
...
int main() {
    ...
    int **mat_bi = (int**) malloc(sizeof(int*) * l);
    for (i=0; i<l; i++){
        mat_bi[i] = (int*)malloc(sizeof(int)*c);
        for(j=0; j<c; j++){
            scanf("%d", & mat_bi[i][j]);
        }
    }

    matrix =converter(l, c, mat_bi);

    printf ("%d",cont_N_null(matrix,l, c));
